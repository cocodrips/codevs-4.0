    ...
    def neet(self, force, forces, resources):
        if resources and len(forces) > 10 and force.type == UnitType.ASSASSIN:
            ...
            if self.brain.aStage.turnNum % GROUP_INTERVAL == 0:
                ...
                if int(self.brain.aStage.turnNum % (GROUP_INTERVAL * 4) < 2) and self.brain.aStage.supporter.aroundStrength(
                    self.brain.castle.point, DEFENCE_RANGE) < self.brain.aStage.enemies.aroundStrength(self.brain.castle.point,
                                                                                           DEFENCE_RANGE):
                    force.forceType = ForceType.GATEKEEPER
                else:
